...
void permute_kernel(sycl::nd_item<1> id, float* q, float* k, float* v,
    ...
    if (idx < B * NH * N * d) {
        ...
        // Replace with streaming loads later
        q[idx] = inp[inp_idx];
...
void unpermute_kernel(sycl::nd_item<1> id, const float* inp, float *out, int B, int N, int NH, int d) {
   // out has shape (B, nh, N, d) but we need to unpermute it to (B, N, nh, d)
    ...
    if (idx < B * NH * N * d) {
        ...
        // Replace with streaming load later
...
void layernorm_forward(float* out, float* mean, float* rstd,
                       ...
                       int B, int T, int C) {   
    const int block_size = 256; // Figure out best block size?
    const int N = B * T;
...
void matmul_forward_cublas(float* out,
    ...
    const int sqrt_block_size = 16; // 32 exceeds Intel GPU block size limits
...
float* malloc_and_point_parameters(ParameterTensors* params, size_t* param_sizes, int on_device) {
    ...
    float* params_memory;
    if (on_device) {
        params_memory = sycl::malloc_device<float>(num_parameters, *DefaultQueue);
        syclMallocCheck(params_memory);
    } else {
...
float* malloc_and_point_activations(ActivationTensors* acts, size_t* act_sizes) {
    ...
    float* acts_memory;
    acts_memory = sycl::malloc_device<float>(num_activations, *DefaultQueue);
    syclMallocCheck(acts_memory);
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path) {
    ...
    DefaultQueue->memcpy(model->params_memory, params_memory_cpu, num_parameters * sizeof(float));
    DefaultQueue->wait();
...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, int B, int T) {
    ...
    if(model->acts_memory == NULL) {
        ...
        model->inputs = sycl::malloc_device<int>(B * T, *DefaultQueue);
        model->targets = sycl::malloc_device<int>(B * T, *DefaultQueue);
        model->cpu_losses = sycl::malloc_host<float>(B * T, *DefaultQueue);
        syclMallocCheck(model->inputs);
        syclMallocCheck(model->targets);
        syclMallocCheck(model->cpu_losses);
    } else {
    ...
    if (targets != NULL) {
        ...
        DefaultQueue->memcpy(model->cpu_losses, acts.losses, B * T * sizeof(float));
        DefaultQueue->wait();
...
void gpt2_zero_grad(GPT2 *model) {
    if (model->grads_acts_memory != NULL) { DefaultQueue->memset(model->grads_acts_memory, 0, model->num_activations * sizeof(float)); }
    if (model->grads_memory != NULL) { DefaultQueue->memset(model->grads_memory, 0, model->num_parameters * sizeof(float)); }
}
...
void gpt2_backward(GPT2 *model) {
    ...
    DefaultQueue->memset(grads_acts.losses, dloss_mean, B*T * sizeof(float));
...
void dataloader_init(DataLoader *loader, const char* filename, int B, int T) {
    ...
    // This is a bit of a hack, as we are not using CUDA, but it works for now
    loader->batch = sycl::malloc_host<int>(B * T + 1, *DefaultQueue);
    syclMallocCheck(loader->batch);
...
int main() {
    ...
    // look into dnnl_fpmath_mode_tf32?
    ...
    for (int step = 0; step <= 40; step++) {
        ...
        if (step > 0 && step % 20 == 0) {
            ...
            for (int t = 1; t < genT; t++) {
                ...
                DefaultQueue->memcpy(cpu_probs, probs, model.config.vocab_size * sizeof(float));
                DefaultQueue->wait();
