...
void crossentropy_softmax_backward1(sycl::queue& queue, float* dlogits,
                           const float* dlosses, const float* probs, const int* targets,
                           int B, int T, int V,
...
void crossentropy_softmax_backward(sycl::queue& queue, int kernel_num,
                           float* dlogits,
                           const float* dlosses, const float* probs, const int* targets,
                           int B, int T, int V,
                           const int block_size) {
    switch (kernel_num) {
        case 1:
            crossentropy_softmax_backward1(queue, dlogits, dlosses, probs, targets, B, T, V, block_size);
            break;
        default:
...
int main(int argc, char **argv) {
    ...
    d_dlosses = sycl::malloc_device<float>(B * T, defaultQueue);
    d_dlogits = sycl::malloc_device<float>(B * T * V, defaultQueue);
    ...
    for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
        ...
        crossentropy_softmax_backward(defaultQueue, kernel_num, d_dlogits, d_dlosses, d_probs, d_targets, B, T, V, block_size);
        validate_result(defaultQueue, d_dlogits, dlogits, "dlogits", B * T * V, 1e-5f);
    }
    ...
    for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
        ...
        float elapsed_time = benchmark_kernel(defaultQueue, repeat_times, crossentropy_softmax_backward,
